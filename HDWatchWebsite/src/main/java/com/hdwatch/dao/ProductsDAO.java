...
public interface ProductsDAO extends JpaRepository<Products, Integer> {
	...
	@Query(value = "SELECT p.* FROM products p " + "LEFT JOIN (SELECT product_id, COUNT(*) AS order_count "
			+ "           FROM orderdetails " + "           GROUP BY product_id) od " + "ON p.id = od.product_id "
	...
	@Query(value = "SELECT p.* FROM products p " + "INNER JOIN categories c ON p.category_id = c.id "
			+ "INNER JOIN brands b ON p.brand_id = b.id " + "WHERE (LOWER(p.name) LIKE %:keyword% "
			+ "OR LOWER(p.description) LIKE %:keyword% " + "OR LOWER(c.name) LIKE %:keyword% "
			+ "OR LOWER(b.name) LIKE %:keyword%) " + "AND (:category IS NULL OR p.category_id = :category) "
			+ "AND (:brand IS NULL OR p.brand_id = :brand)", countQuery = "SELECT count(*) FROM products p "
					+ "INNER JOIN categories c ON p.category_id = c.id " + "INNER JOIN brands b ON p.brand_id = b.id "
					+ "WHERE (LOWER(p.name) LIKE %:keyword% " + "OR LOWER(p.description) LIKE %:keyword% "
					+ "OR LOWER(c.name) LIKE %:keyword% " + "OR LOWER(b.name) LIKE %:keyword%) "
