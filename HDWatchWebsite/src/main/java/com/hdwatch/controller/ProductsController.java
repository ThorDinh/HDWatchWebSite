...
public class ProductsController {
	...
	public String search(@RequestParam(name = "keyword", required = false) String keyword,
			...
			@RequestParam(name = "brand", required = false) Integer brand, @RequestParam("p") Optional<Integer> p,
			Model model) {
		// Tiêu đề trang
		...
		if (keyword != null && !keyword.trim().isEmpty() || category != null || brand != null) {
			...
			if (keyword.equalsIgnoreCase("")) {
				...
				model.addAttribute("message",
						"Có " + filteredProducts.getTotalElements() + " kết quả trả về cho từ khóa: " + keyword);
			}
		} else {
			// Tiêu đề trang
			model.addAttribute("pageTitle", "Tất cả sản phẩm");
